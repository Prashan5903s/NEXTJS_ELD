...
const LocationTable = () => {
    ...
    const fetchPermissions = useCallback(debounce(async () => {
        try {
            const perms = await getPermissions();
            setPermissions(perms);
        } catch (error) {
    ...
    const columns = React.useMemo(
        () => [
            ...
            { header: 'Address Type', accessorKey: 'address_type' },
            { header: 'High Accelerating Setting Type', accessorKey: 'model' },
                                    ...
                                    <tbody className="fw-semibold text-gray-600">
                                        {locationData && locationData.map(data => (
                                            <tr key={data.id}>
                                                ...
                                                <td>{data.model}</td>
                                                <td>{data.year}</td>
                                                <td>{data.harsh_acceleration_setting_type}</td>
                                                <td>{data.notes}</td>
                                                <td>
                                                    <div className={`badge badge-light-${data.status ? 'success' : 'danger'}`}>
                                                        {data.status ? 'Active' : 'De-active'}
