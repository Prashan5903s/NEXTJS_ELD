...
const AddDocumentModal = ({ id, close, open, updateDocumentList }) => {
        ...
        for (const [key, value] of Object.entries(formValidations)) {
            if (key === "file") {
                ...
                if (file) {
                    ...
                    const allowedFormats = ["image/jpeg", "image/png", "application/pdf", "application/msword"];
    ...
    const debouncedSubmit = useCallback(
        ...
        [url, updateDocumentList, router, close] // Ensure only `url` is included as dependency
    );
    ...
    const fetchEditData = useCallback(
            ...
            try {
                ...
                if (response.data && response.data.doc) {
                    ...
                    setDocumentField({
                        ...
                        file: null, // Reset file field, file uploads are handled differently
        ...
        [url] // Make sure dependencies like 'url' are passed correctly
    );
