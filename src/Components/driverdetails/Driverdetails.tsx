...
export default function Driverdetails() {
  ...
  function getCookie(name: string): string | null {
    if (typeof window === "undefined") {
      // If window is not defined, it's on the server side
      return null;
    }
  ...
  }, [slug, fetchData]); // Dependencies for useEffect
  ...
  const fetchGraphData = useCallback(
    ...
    [token, BackEND] // Memoized dependencies
  );
      ...
      <div className="row">
        <div className={`col-6 ${classes.leftSideScroll}`}>
          ...
          <h3 className="align-items-start flex-column fs-2 fw-bold text-gray-800 mt-5">
            {data && data[0] && data[0].first_name}{" "}
            {data && data[0] && data[0].last_name}
          </h3>
          <div className="mb-4">
